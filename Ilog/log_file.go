...
func WithRotationType(t int) Option {
	return OptionFn(func(rl *RotateLogs) error {
		rl.rotationType = t
		return nil
	})
...
func NewWriter(filePath string, options ...Option) (*RotateLogs, error) {
	var rl RotateLogs
	...
	if rl.rotationType == 0 {
		rl.rotationType = rotationTypeDay
	}

	pattern := "";
	switch rl.rotationType {
	case rotationTypeYear:
		pattern = filePath + ".%Y.log"
	case rotationTypeMonth:
		pattern = filePath + ".%Y%m.log"
	case rotationTypeDay:
		pattern = filePath + ".%Y%m%d.log"
	case rotationTypeHour:
		pattern = filePath + ".%Y%m%d%H.log"
	case rotationTypeMinute:
		pattern = filePath + ".%Y%m%d%H%M.log"
	}
