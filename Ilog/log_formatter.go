...
func (f *TextFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	...
	if isFormatted {
		...
		//f.appendKeyValue(b,"file",file.Name(),true)
		//f.appendKeyValue(b,"line",line,true)
		//
		//if entry.Message != "" {
		//	f.appendKeyValue(b, "msg", entry.Message, lastKeyIdx >= 0)
		//}

		log := fmt.Sprintf("%s - %s - [%s:%d]:%s", entry.Time.Format(timestampFormat), entry.Level.String(), entry.Data["[method]"], entry.Data["[line]"], entry.Message)
		b.WriteString(log + " ")
		for i, key := range keys {
			if key == "[line]" || key == "[method]" {
				continue
			}
...
func (f *TextFormatter) printColored(b *bytes.Buffer, entry *logrus.Entry, keys []string, timestampFormat string, colorScheme *compiledColorScheme) {
	...
	if f.DisableTimestamp {
		fmt.Fprintf(b, "%s%s [%s:%s]"+messageFormat, level, prefix, message, file.Name(), line)
	} else {
		...
		fmt.Fprintf(b, "%s %s%s [%s:%d] "+messageFormat, colorScheme.TimestampColor(timestamp), level, prefix, entry.Data["[method]"], entry.Data["[line]"], message)
	}
	for _, k := range keys {
		if k != "prefix" && k != "[line]" && k != "[method]" {
			v := entry.Data[k]
